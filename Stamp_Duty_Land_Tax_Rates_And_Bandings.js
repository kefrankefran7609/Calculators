...
fetch("https://api.airtable.com/v0/apphKFqKdClqcgF26/tblIt28vfTJiKpCe1", requestOptions)
  ...
  /* Give the values to the elements on the page from our APi call */
   ...
   document.querySelector('[second]').textContent = parsed.records[4].fields["Purchase consideration"]
   document.querySelector('[secondrate1]').textContent = parsed.records[4].fields["Rate – on portion above threshold"]
   document.querySelector('[secondrate2]').textContent = parsed.records[4].fields["Rate if second or subsequent property"]
